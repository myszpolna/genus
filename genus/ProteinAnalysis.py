...
class ProteinAnalysisWorker(Worker):
    ...
    def map(self):
        ...
        self.mini, self.maxi, self.length, self.nr_chord = self.__compute_info(self.cl_data)
        self.no_biff = self.__change(self.remove_biff(self.cl_data))
        self.b1_b2_list = self.__multiplicity(self.cl_data,self.mini,self.maxi)
        self.genuses = self.devide_and_compute(self.mini,self.maxi,self.no_biff)
